...
public class UserService {
    ...
    public boolean isSamePassword(String existPassword, String newPassword) {
    ...
    public String loginUser(User user, String password) {
        log.info("Attempting to log in user with email: {}", user.getEmail());


        if (!user.isEmailConfirmed()) {
            log.warn("User with email '{}' is not confirmed. Throwing exception.", user.getEmail());
            throw new EmailNotConfirmedException(user.getEmail());
        }

        log.debug("Checking password for user '{}'", user.getEmail());
        if (!isSamePassword(user.getPassword(), password)) {
            log.warn("Invalid credentials provided for user '{}'. Throwing exception.", user.getEmail());
            throw new InvalidCredentialsException();
        }

        log.info("User with email '{}' successfully logged in. Generating token...", user.getEmail());
